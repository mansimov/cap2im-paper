...
${\bf y} = \{y_{1}, y_{2}, ..., y_{N}\}$, 
where $K$ is the size of he vocabulary and $N$ is the length of the sequence.
...
sequence of latent variables $\Lat_t \in \mathbb{R}^D$, $t=1,..,T$, where the output is accumulated over all $T$ timesteps.
...
the learnt model parameters, where $n$ is the dimensionality of $\hdec_{t}$,
...
\label{eq:x_hat}
\lat_t &\sim \prior(\Lat_t|\Lat_{1:t-1}) = \mathcal{N}\bigg(\mu_{t}(\hdec_{t-1}), \sigma_{t}(\hdec_{t-1})\bigg),\\
s_{t} &= \aalign(\hdec_{t-1}, \hlangall), \\
\hdec_t &= \decoder(\hdec_{t-1}, [z_t, s_{t}]), \\
\canv_t &= \canv_{t-1} + \writeop(\hdec_t), \\
\tilde{\oimage} &\sim P(\oimage\given\icaption, \Lat_{1:T}) = \prod_i P(x_i\given\icaption, \Lat_{1:T}) = \prod_i \sigmoid(\canv_{T,i}),
\label{eq:write}
%\canv_t &= \canv_{t-1} + \writeop(\hdec_t)
\end{align}
where $\decoder$
is the function defined by the LSTM network with \textit{forget} gates 
\citep{gers_forget} at a single time-step. To generate the next 
hidden state $\hdec_t$, 
the decoder $\decoder$ takes its previous hidden state $\hdec_{t-1}$ and
combines it with the input from both the sampled latent variable $z_t$ and 
caption representation $s_t$ at time-step $t$, where
$[v_1, v_2]$ denotes the concatenation of the vectors $v_1$ and $v_2$ into the single vector.
The initial state of the generative LSTM is set to $\hdec_0 = \tanh(b)$, with learnt bias $b$. 
...
$\alpha_{1...N}^{t}$:
\beqa
s_t=\aalign(\hdec_{t-1}, \hlangall) = \alpha_{1}^{t}\hlang_{1} + \alpha_{2}^{t}\hlang_{2} + ... + \alpha_{N}^{t}\hlang_{N}.
\eeqa
The corresponding alignment probabilities $\alpha_{1...N}^{t}$ at each step are obtained using the caption representation $\hlangall$ and the hidden state of the generative model $\hdec_{t}$, by first calculating energies using a $\tanh$ perceptron scaled by vector $\vv$ 
and then normalizing them: %, as in \citep{bahdanau_mt}:
\beqa
\alpha_{k}^{t}=\frac{\exp(\vv^{\top}\tanh(U\hlang_{k} + W\hdec_{t} + b)}{\sum_{j=1}^{N}
\exp(\vv^{\top}\tanh(U\hlang_{j} + W\hdec_{t} + b)}, 
\eeqa
where $U,W,b$ are the learned model parameters. 

%is a standard \textit{Long Short-Term Memory} architecture \citep{hochreiter_lstm} with \textit{forget} gates \citep{gers_forget}, $[v_1, v_2]$ is the concatenation of the vectors $v_1$ and $v_2$ into the single vector, and $\canv_0$, $\hdec_0$ are initialized to the the learned biases. 

The $\writeop$ operator predicts two arrays of Gaussian filters using a parametric mapping $\gFilterx:\real$ applies an array of Gaussian filters on the patch that places it on the canvas $c_t$ with smoothly varying location and zoom (same as in \citep{gregor_draw}), with 
$c_0=0$. 
\beqa
\label{eq:write}
...
Finally, the $i^{th}$ pixel $\canv_{T,i}$ from the final canvas $\canv_T$ is transformed using a sigmoid function $\sigmoid$ to produce a conditional Bernoulli data likelihood over the image pixels $\oimage$ given the latent variables $\Lat$ and the input caption $\icaption$,
