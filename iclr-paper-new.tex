...
\beqa
\loss = \sum_{\Lat}Q(\Lat\given\oimage,\icaption)\left(\log P(\oimage\given\icaption, \Lat_{1:T}) - \kldiv\left(Q(\Lat\given\oimage,\icaption)\klBars 
...
Similar to the DRAW network, the inference RNN produces an approximate posterior $Q(\Lat_{1:T}\given\oimage,\icaption)$ via a $\readop$ operator, which reads a patch from image using an array of Gaussian filters (inverse of $\writeop$), over the training image at each step. 
\begin{align}
\label{eq:x_hat}
