...
We showed that our proposed model managed to generate reasonably looking images corresponding to respective captions as well as managed to generalize to completely novel scenarios which are very unlikely to occur in real life. Sequential generation of images over the number of steps using an attention mechanism has played an important part in this process. However, the tendency of the conditional DRAW models to underfit on the training data has resulted in blurry samples. 
As a result, the samples were sharpened by a LAPGAN trained on the residuals of the images. Unfortunately, this is not a quite satisfying solution due to the fact that the whole model wasn't trained in an end-to-end fashion. We think that a further extension of the capabilities of the sequential generation of images might help the variational models better fit the data and escape the curse of blurriness.
