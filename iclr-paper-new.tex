...
\end{align}
where $W_{\mu}, W_{\sigma}$ are learnt model parameters. It has been shown in \citep{bachman_sdm} that model performance is improved by including dependencies between latent variables. An image is generated by iteratively computing the following equations for $t=1,...,T$ (see Figure~\ref{fig:figmodel}):
\begin{align}
\label{eq:x_hat}
