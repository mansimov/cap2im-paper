...
where $\hat{\oimage}$ is the \textit{error} image, $\sigmoid$ operator is a logistic sigmoid function $\sigmoid=\frac{1}{1+exp(-x)}$ and $\henc_0$ is initialized to the learned bias. 
