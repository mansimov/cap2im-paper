...
with learned bias $b$. \FIXME{(Elman), actually I checked my code and I found that I didn't tanh the learned bias. Luckly, the values of the bias are very close to zeros (0.0017). Next time should be more careful}.

The output of the decoder $\hdec_t$ is then passed through the $\writeop$ operator 
which is added to a cumulative canvas matrix $c_t$, with $c_0=0$ \FIXME{(Elman) $c_0$ is also initialized to learned bias also very close to zeros} (\Eqref{eq:writeop}). 
