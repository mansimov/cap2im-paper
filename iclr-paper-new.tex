...
The conditional DRAW network is a stochastic recurrent neural network that consists of a 
sequence of latent variables $\Lat_t \in \mathbb{R}^D$, $t=1,..,T$, where the output is accumulated over all timesteps.
...
%Namely, the mean and variance of prior distribution $\prior(\Lat_t)$ are parameterized by: 
%\begin{align}
%\mu_{t}(\hdec_{t-1}) &= tanh(W_{\mu}\hdec_{t-1}),\\
%\sigma_{t}(\hdec_{t-1}) &= \exp\big(tanh(W_{\sigma}\hdec_{t-1})\big),
%\end{align}
%where $W_{\mu}, W_{\sigma}$ are learnt model parameters. 
...
An image is generated by iteratively computing the following equations for $t=1,...,T$ (see Figure~\ref{fig:figmodel}):
\begin{align}
\label{eq:x_hat}
...
%\end{align}
The $i^{th}$ pixel $\canv_{T,i}$ from the final canvas $\canv_T$ is transformed using a sigmoid function $\sigmoid$ to produce a conditional Bernoulli data likelihood over the image pixels $\oimage$ given the latent variables $\Lat$ and the input caption $\icaption$,
