...
%\canv_t &= \canv_{t-1} + \writeop(\hdec_t)
\end{align}
where $\writeop$ is the same attention operator as in \citep{gregor_draw}.
...
The generated images with the attention alignments are displayed in Figure~\ref{fig:figmnist}.
...
We showed that our proposed model managed to generate reasonable images corresponding to respective captions as well as managed to generalize to completely novel scenarios which are very unlikely to occur in real life. Sequential generation of images using attention played a crucial part in it. However the conditional DRAW model still underfits on training data which leads to the blurry samples. We think that extending this process by generating very small sections of images over potentially hundreds of timesteps might help the model fit the data and generate sharper images.
