...
To generate images conditioned on the caption information, we extended the DRAW network \citep{gregor_draw} to include caption representation $\hlangall$ at each step, shown in Figure~\ref{fig:figmodel}. The conditional DRAW network is a stochastic recurrent neural network that consists of a set of latent variables $\Lat_t$ where the output is accumulated over all time steps.
...
%\end{align}
The corresponding alignment probabilities $\alpha_{1...N}$ at each step are obtained using the caption representation $\hlangall$ and the hidden state of the generative model $\hdec_{t}$, by first calculating energies $e_{1...N}$ using a $tanh$ perceptron and then normalizing them, as in \citep{bahdanau_mt}:
%\begin{align}
$\alpha_{k}=\frac{exp(v^{T}tanh(U\hlang_{k} + W\hdec_{t} + b)}{\sum_{j=1}^{N}exp(v^{T}tanh(U\hlang_{j} + W\hdec_{t} + b)}.$
%\end{align}
The final canvas $\canv_T$ is transformed using a sigmoid function to produce a conditional Bernoulli data likelihood over pixels given the latent variables and the input caption.
...
In this paper, we have demonstrated that our alignDraw model, a combination of a sequential variational autoencoder and a alignment model over words, succeeds in generating images that correspond to a given input caption. By making extensive use of attentional mechanisms, our model gains several advantages. Namely, the use of the visual attention mechanism allows us to decompose the problem of image generation into a series of steps instead of a single forward pass, while the attention over words can provide us insight whenever our model fails to generate a relevant image. Additionally, because our model conditions on unstructured textual descriptions, we can generate images corresponding to captions that generalize beyond the training set such as sentences describing completely novel scenarios which are highly unlikely to occur in real life. 
Because we have observed that the alignDraw model tends to output slightly blurry samples, likely due to underfitting on the training data, we augmented the model with a sharpening post-processing step in which a LAPGAN generates edges that are added to the alignDraw samples. 
Unfortunately, this is not an ideal solution due to the fact that the whole model is not trained in an end-to-end fashion. Therefore a direction of future work would be to find methods that can the bypass the separate post-processing step and output sharp images directly in an end-to-end manner. 
