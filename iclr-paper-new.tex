...
Deep Neural Networks have achieved significant success in various tasks such as image recognition \citep{krizhevsky_imagenet}, speech transcription \citep{graves_speech} and etc. 
...
%\canv_t &= \canv_{t-1} + \writeop(\hdec_t)
\end{align}
where $\writeop$ is the same attention operator as in \citep{gregor_draw}. The $\writeop$ operator applies an array of 2D Gaussian filters on the patch that places it on the canvas with smoothly varying location and zoom.
...
%\end{align}
The corresponding alignment probabilities $\alpha_{1...N}$ at each step are obtained using the caption representation $\hlangall$ and the hidden state of the generative model $\hdec_{t}$, by first calculating energies $e_{1...N}$ using $tanh$ perceptron and then normalizing them, as in \citep{bahdanau_mt}:
%\begin{align}
$\alpha_{k}=\frac{exp(v^{T}tanh(U\hlang_{k} + W\hdec_{t} + b)}{\sum_{j=1}^{N}exp(v^{T}tanh(U\hlang_{j} + W\hdec_{t} + b)}.$
%\end{align}
The final canvas $\canv_T$ is transformed using a sigmoid function to produce an Bernoulli data likelihood over pixels given the latent variables and the input caption.
...
The generated images together with the attention alignments are displayed in Figure~\ref{fig:figmnist}. The model correctly displayed defined digits at the described positions and even managed to generalize reasonably to the configurations that were never present during training. In case of generating two digits, the model attended to the digit in the caption that was drawn at the respective timestep while completely ignoring the other digit. As for generating one digit, the model attended to the digit in the caption during the whole generation process. However in both cases, the model placed small attention values on the words describing the position of digits in the images.
...
very challenging. For consistency with related work on caption generation, we used only first five captions when training and evaluating our model. The images were resized to $32 \times 32$ dimension for consistency with other tiny images datasets \citep{krizhevsky_cifar}. In the following subsections, we analyzed both the qualitative and quantitative aspects of our model as well as compared its performance with that of other, related generative models\footnote{To see more generated images, go to \url{http://www.cs.toronto.edu/~emansim/cap2im.html}}.
...
LAPGAN & - & - & - & - & - & 0.08 \\ % - & - & - & - & - & 0.08
\hline
Fully-Conn VAE & 1.0 & 6.6 & 12.0 & 53.4 & 47 & 0.156 \\ % 0.688 & 4.58 & 8.74 & 46.832 & 54 & -1
Conv-Deconv VAE & 1.0 & 6.5 & 12.0 & 52.9 & 48 & 0.164 \\ % 0.672 & 4.448 & 8.476 & 46.596 & 54 & 0.131
skipthoughtDRAW & 2.0 & 11.2 & 18.9 & 63.3 & 36 & 0.157 \\ % 0.988 & 6.18 & 11.48 & 52.512 & 48 & 0.118
noalignDRAW & 2.8 & 14.1 & 23.1 & 68.0 & 31 & 0.155 \\ % 1.0 & 6.124 & 11.18 & 52.112 & 48 & 0.114
alignDRAW & 3.0 & 14.0 & 22.9 & 68.5 & 31 & 0.156 \\ % 0.924 & 6.216 & 11.184 & 52.492 & 48 & 0.115
\end{tabulary}
\end{center}
...
We showed that our proposed model managed to generate reasonably looking images corresponding to respective captions as well as managed to generalize to completely novel scenarios which are very unlikely to occur in real life. Sequential generation of images over the number of steps using attention mechanism has played an important part in this process. However, the tendency of the conditional DRAW models to underfit on the training data has resulted in the blurry samples. As a result, the samples were sharpened by the GAN trained on the residuals of the images. Unfortunately, this is not a quite satisfying solution due to the fact that the whole model wasn't trained in the end-to-end fashion. We think that further extention of the capabilities of the sequential generation of images might help the variational models better fit the data and escape the curse of blurriness.
