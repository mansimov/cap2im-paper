...
Unlike the original DRAW network where latent variables are independent unit Gaussians $\mathcal{N}(0, I)$, the latent variables in the proposed alignDRAW model have their mean and variance depend on the previous recurrent hidden states $\hdec_{t-1}$, except for $\lat_1 \sim \prior(\Lat_1) = \mathcal{N}(0, I)$.  Namely, the mean and variance of prior distribution $\prior(\Lat_t)$ are parameterized by: 
\begin{align}
\mu_{t}(\hdec_{t-1}) &= tanh(W_{\mu}\hdec_{t-1}),\\
\sigma_{t}(\hdec_{t-1}) &= {exp}\big(tanh(W_{\sigma}\hdec_{t-1})\big),
\end{align}
where $W_{\mu}, W_{\sigma}$ are learnt model parameters. It has been shown in \citep{bachman_sdm} that model performance is improved by including dependencies between latent variables. Formally, the image is generated by iteratively computing the following equations for $t=1,...,T$ (see Figure~\ref{fig:figmodel}):
\begin{align}
\label{eq:x_hat}
\lat_t &\sim \prior(\Lat_t|\Lat_{1:t-1}) = \mathcal{N}\big(\mu_{t}(\hdec_{t-1}), \sigma_{t}(\hdec_{t-1})\big),\\
...
%\end{align}
The corresponding alignment probabilities $\alpha_{1...N}^{t}$ at each step are obtained using the caption representation $\hlangall$ and the hidden state of the generative model $\hdec_{t}$, by first calculating energies using a $tanh$ perceptron scaled by vector $v$ and then normalizing them, as in \citep{bahdanau_mt}:
%\begin{align}
%\vspace{-0.1in}
$\alpha_{k}^{t}=\frac{exp(v^{T}tanh(U\hlang_{k} + W\hdec_{t} + b)}{\sum_{j=1}^{N}exp(v^{T}tanh(U\hlang_{j} + W\hdec_{t} + b)}.$
%\end{align}
The final canvas $\canv_T$ is transformed using a sigmoid function to produce a conditional Bernoulli data likelihood over the image pixels $\oimage$ given the latent variables $\Lat$ and the input caption $\icaption$,
\begin{align}
P(\oimage\given\icaption, \Lat_{1:T}) = \prod_i P(x_i\given\icaption, \Lat_{1:T}) = \prod_i \sigmoid(\canv_{T,i}),
\end{align}
where, $\canv_{T,i}$ is the $i^{th}$ pixel on the final canvas computed at step T and $\sigmoid$ operator is a logistic sigmoid function $\sigmoid(x)=\frac{1}{1+exp(-x)}$. 
\comm{
